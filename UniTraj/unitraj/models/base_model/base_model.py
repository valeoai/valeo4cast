...
class BaseModel(pl.LightningModule):
    ...
    def compute_pred_trajs_world(self, batch_dict, prediction):
    ...
    def compute_official_evaluation(self, batch_dict, prediction):
        if self.config.get('eval_waymo', False):
            ...
            pred_trajs_world = self.compute_pred_trajs_world(batch_dict, prediction)
    ...
    def log_info(self, batch, prediction, status='train'):
        ...
        # print("gt_traj_mask ", gt_traj_mask.sum(1).sum(1))
        #TODO update xyh: make sure having at least one future
       # assert any(gt_traj_mask.sum(1).sum(1)>0)
        ...
        #TODO xyh in case no future, add torch.clamp_min
        ade_losses = torch.sum(ade_diff * gt_traj_mask, dim=-1) / torch.clamp_min(torch.sum(gt_traj_mask, dim=-1), min=1.0) 
        ...
        if not any(gt_traj_mask.sum(1).sum(1)>0):
            #print("center_gt_final_valid_idx", center_gt_final_valid_idx)
            #print("minfde", minfde)
            minfde = minfde*0+50
        best_fde_idx = np.argmin(fde, axis=-1)
        predicted_prob = predicted_prob[np.arange(bs), best_fde_idx]
        miss_rate = (minfde > 2.0)
        brier_fde = minfde + (1 - predicted_prob)**2
        ...
        if status == 'val' and self.config.get('eval', False):
            ...
            new_dict = {}
            agent_types = [1, 2, 3, 4]
            agent_type_dict = {1: "vehicle", 2: "pedestrian", 3: "bicycle", 4:'other'}
