...
class BaseDataset(Dataset):
    ...
    def __init__(self, config=None, is_validation=False):
        ...
        if self.config["drop_no_future"] == True:
            print("!!!!!!!!!!!!!!Warning: you drop all objects having no future. Might cause FN in forecasting with noisy inputs.!!!!!!!!!!!!!!!!")
    ...
    def load_data(self):
        ...
        for cnt, data_path in enumerate(self.data_path):
            ...
            if self.config["debug_challenge"]:
                ...
                file_list = self.process_data_chunk(0)
            else:
    ...
    def process_data_chunk(self, worker_index):
        ...
        for cnt, file_name in enumerate(data_list):
            if self.config["debug_challenge"]:
                if "02a00399-3857-444e-8db3-a8f58489c394" not in file_name:
                    continue
...
#            stop_point = stop_point[:total_steps]
    ...
    def process(self, internal_format):
        ...
        for current_time_index in all_time_indices:
            
            #TODO update xyh: drop last frame which surely have no future
            if self.config["drop_no_future"] and current_time_index == nb_all_ts - 1:
                if self.config["debug_challenge"]:
            ...
            # center_objects is (num_objects_of_interest, 10)
            ...
            if self.config["debug_challenge"]:
                print("before center_objects: ", len(center_objects))
                print("before track_index_to_predict: ", len(track_index_to_predict))
            ...
            obj_trajs_past_at_current_time = obj_trajs_full[:, starting_past_ts:current_time_index+1]
            obj_trajs_future_at_current_time = obj_trajs_full[:, current_time_index+1:ending_future_ts]
            ...
            if self.config["debug_challenge"] and current_time_index == 35:
                print("obj_trajs_past_at_current_time[20] ", obj_trajs_past_at_current_time[20, 20])
            ...
            new_reindexing = np.zeros(obj_trajs_full.shape[0], dtype=np.int32)-1 # {old index: nex index}
            ...
            if self.config["debug_challenge"] and current_time_index == 35:
                print("new_reindexing[20] ", new_reindexing[20])
                print("new_reindexing[track_index_to_predict] ", new_reindexing[track_index_to_predict])
                print("obj_trajs_full_at_current_time ", obj_trajs_full_at_current_time[new_reindexing[20], 20])
            ####
            ...
            (obj_trajs_data, obj_trajs_mask, obj_trajs_pos, obj_trajs_last_pos, obj_trajs_future_state,
             ...
             track_index_to_predict_new) = self.get_agent_data(
                ...
                timestamps=timestamps, obj_types=obj_types_at_current_time
            )
        ...
        ret_dict = {
            ...
            'track_index_to_predict': track_index_to_predict_new,  # used to select center-features # usually it's always 0, 0, 0...
