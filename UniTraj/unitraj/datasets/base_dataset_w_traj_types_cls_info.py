        ...
        for cnt, file_name in enumerate(data_list):
            ...
            if "need_match_with_gt" in self.config and self.config['need_match_with_gt']:
                if '/val/' in data_path and 'merge' not in data_path:
                    print("in GT val.")
                    data_path_GT = "/home/yxu1/scania/datasets_scania/argoverse2_sensor_scenarios/val/"
                    _, _, mapping_GT = read_dataset_summary(data_path_GT)
                elif '/train/' in data_path and 'merge' not in data_path:
                    data_path_GT = "/home/yxu1/scania/datasets_scania/argoverse2_sensor_scenarios/train/"
                    print("in GT train.")
                    _, _, mapping_GT = read_dataset_summary(data_path_GT)
                else:
                    print("in GT merge.")
                    data_path_GT = "/home/yxu1/scania/datasets_scania/argoverse2_sensor_scenarios/merge/"
                    mapping_GT = {}
                    _, _, mapping_GT_train = read_dataset_summary("/home/yxu1/scania/datasets_scania/argoverse2_sensor_scenarios/train/")
                    for k,v in mapping_GT_train.items():
                        assert k not in mapping_GT
                        mapping_GT[k] = 'train/' + v + '/'
                        
                    _, _, mapping_GT_val = read_dataset_summary("/home/yxu1/scania/datasets_scania/argoverse2_sensor_scenarios/val/")
            ...
            if self.config["debug_challenge"]:
                print("processing...")
            output = self.process(output, output_GT)
...
#            stop_point = stop_point[:total_steps]
    ...
    def process(self, internal_format, internal_format_GT):
        ...
        for time_idx, current_time_index in enumerate(all_time_indices):
            ...
            if "need_match_with_gt" in self.config and self.config["need_match_with_gt"]:
                ...
                valid_past_preds_idx = np.where(obj_trajs_past_at_current_time[:,-1, -1]==1)[0]
                ...
                valid_past_gt_idx = np.where(obj_trajs_past_at_current_time_GT[:,-1, -1]==1)[0]
                ...
                assert validity_past_gts.sum() > 0.0
                ...
                for valid_idx, valid_past_pred in enumerate(valid_past_preds):
                    ...
                    # get past_dist
                    # print("valid_past_pred[:, :3] ", valid_past_pred[:, :3])
                    # print("valid_past_gts[:,:,:3] ", valid_past_gts[:,:,:3])
                    # print("validity_past_gts.shape ", validity_past_gts.shape)
                    # print("validity_past_pred.shape ", validity_past_pred.shape)
                    # :2 => (x,y)
                    ...
                    if min_past_dist <= 2.0:
                        ...
                        obj_trajs_future_at_current_time[pred_index_in_full,:,:] = obj_trajs_future_at_current_time_GT[GT_index_in_full,:,:]
            ...
            if self.config["drop_no_future"]:
                if "need_match_with_gt" in self.config and self.config["need_match_with_gt"]: # softer condition for noisy inputs
                    # print("here.")
